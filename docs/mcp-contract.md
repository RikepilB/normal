...
- A WhatsApp Recipient Exclusion is enforced beneath every grant and never changes the grant itself. While one applies, the recipient is absent from `list_contacts`, `list_groups`, and `list_chats`; `read_messages` and Stored Media resources reject a retained WhatsApp Conversation handle or media URI as not found; and an unbound `send_text_message`, `send_pdf_file`, or `send_image` returns `recipient_not_found` before quota reservation, source retrieval, or provider access. The retained-binding rules below apply first to replays: a destination-compatible file request may load verified bytes only to return the existing receipt or an idempotency conflict and never reaches quota reservation or provider access. No tool discloses that an exclusion is the reason, and no MCP tool can read or change one.
...
| `messages:send` | `send_text_message`, `send_pdf_file`, `send_image`, `get_send_status` |
...
- `conversation_id` echoes the authorized WhatsApp Conversation read by this result. `recipient_id` is its current `ctc_` or `grp_` WhatsApp Recipient handle and can be passed directly to `send_text_message`, `send_pdf_file`, or `send_image`; `kind` is `direct` or `group`.
...
`send_text_message`, `send_pdf_file`, `send_image`, and `get_send_status` use the same normalized `status` enum:

| Status | Meaning |
| --- | --- |
| `processing` | The platform has durably claimed this one attempt and its type-specific provider work has not reached a known outcome. |
...
For a new send, one Neon transaction creates the Activity Log, Send Operation, idempotency binding, and quota reservation and sets `status: "processing"`, anchors `attempt_claimed_at` at the database insertion after any encrypted Pending Send File snapshot, and sets `lease_expires_at` 45 seconds later. That commit is the durable provider-attempt boundary. Before commit, failure rolls back the operation, binding, and quota and returns `isError`; after commit, quota remains consumed and every observable outcome is an operation receipt. The Worker returns the committed `processing` receipt without waiting for provider latency and attaches the type-specific bounded provider work to its execution context: one 15-second text call, or one 10-second file upload followed by at most one 15-second PDF or image send. Each timeout includes the byte-bounded response-body read, and the lease exceeds the platform's deferred-work ceiling so every provider result the Worker can complete remains recordable before expiry. That deferred attempt uses an isolated database connection once the request connection scope closes. Its complete direct response is causally bound and may advance the operation without a provider message identifier; later or independently delivered webhook evidence may advance it only through ADR 0017's exact same-connection/shared-stable-identifier correlation. Lease expiry atomically changes `processing` to `unknown`; no caller can take over or redispatch it, and a concurrent exact replay returns the current receipt immediately.
...
Exact outbound text or an image caption is retained in active application state as encrypted Pending Send Content only until the earliest of Stored Message creation, definitive pre-send failure, seven days after `created_at`, the Message Retention Policy deadline, or Connection or Personal Account Deletion. A Pending Send File uses the same deadline and immediate pre-send-failure deletion rule in Stored Media storage. For a finite policy duration `D`, `pending_expires_at = min(created_at + 7 days, created_at + D)`; retain-until-deletion contributes no earlier policy deadline, so the seven-day cap still applies. Stored Message creation consumes the applicable pending content and file. After pending content expires, the 90-day binding retains only a keyed, non-reversible exact-request fingerprint for idempotency comparison; `accepted` and `unknown` statuses may outlive readable content and files. Encrypted remnants may remain only inside managed recovery history, and restore expiry gates prevent them from becoming application-readable again.

A direct provider response can update its causally bound Send Operation without a provider message identifier. It can create an outbound text Stored Message only when it carries trusted stable message identity and still-retained Pending Send Content. Pending Send Files are not promoted into Stored Media from identity-only evidence. Independently delivered evidence can associate with an operation only through the same connection and an exact stable provider message identifier that the adapter has verified is shared by both payload types. The encrypted identifier and its connection- and namespace-scoped keyed equality index remain with the operation; recipient, text, timestamp proximity, status order, and apparent candidate uniqueness are never sufficient.

An authenticated outbound upsert creates or updates a Stored Message only when it carries its own trusted stable provider message identifier. Without shared identity it remains independent of any Send Operation. A stronger content-bearing upsert or edit overrides Pending Send Content and a stronger media-bearing upsert overrides a Pending Send File. If identity-only `sent`, `delivered`, or `read` evidence arrives after pending content was removed, including after a corrected `failed`, it updates only the Send Operation; no readable Stored Message is created until a content-bearing identity-bearing upsert arrives before the Message Retention Policy deadline. For an image Send Operation, identity-only evidence never creates a caption without the image; a media-bearing outbound upsert establishes the Stored Message and supersedes the pending image and caption together. Text, captions, and files are never reconstructed from the request fingerprint. `processing`, `accepted`, unresolved `unknown`, and failures before qualifying evidence remain operation-only, while a later failure does not remove a message already established by stronger evidence. Disconnection prevents new sends but does not block convergence of items already queued for ingestion or authenticated, exactly correlated late evidence for retained operations.
...
Requires `messages:send` and the same Client Confirmation, destination, idempotency, durable-attempt, status, Recipient Exclusion, and no-automatic-retry rules as `send_text_message`. It accepts exactly one `recipient_id`, `phone`, or `username`, and exactly one image source: `image_url` or strict standard padded `image_base64`.
...
| `phone` | E.164 string | exactly one destination | Direct phone address beginning with `+`. |
| `username` | string matching `^@[A-Za-z0-9._-]{1,64}$` | exactly one destination | Direct WhatsApp username address. |
| `image_url` | HTTPS URL | exactly one source | Image source without credentials or a custom port. |
| `image_base64` | standard padded Base64 string | exactly one source | Strict Base64 with the standard alphabet, canonical `=` padding where required, and no whitespace or data-URL prefix. |
| `caption` | string, 1-4,096 Unicode scalar values | no | Exact non-blank caption governed by the `send_text_message.text` Unicode rules. |
| `idempotency_key` | string matching `^[A-Za-z0-9_-]{21}$` | yes | Caller-generated NanoID-default-alphabet retry identity governed by ADR 0006. |

Decoded or downloaded content has an exact maximum of `5,000,000` bytes. The server accepts only a JPEG signature beginning `FF D8 FF` or the complete eight-byte PNG signature `89 50 4E 47 0D 0A 1A 0A` and derives normalized MIME as `image/jpeg` or `image/png` from those bytes rather than trusting URL metadata. URL retrieval uses the PDF source rules: every hostname resolves only to global addresses, IP/private/reserved targets are rejected, at most three manually validated HTTPS redirects are followed, and time and actual downloaded bytes bound the complete operation. Caption validation occurs before quota reservation, source retrieval, or Send Operation creation. It uses the exact scalar, nonblank, preservation, and UTF-8 rules of `send_text_message`; absence and an exact present caption are distinct idempotency inputs.

Before provider access, the server snapshots and encrypts the verified image as a Pending Send File in Stored Media storage and attaches its exact size, SHA-256, and normalized MIME to the Send Operation. The file counts against the Personal Account's Stored Media quota and follows existing Pending Send File rollback, retention, deletion, and confirmed-object-deletion quota-release rules. The request fingerprint binds the destination, exact image SHA-256, normalized MIME, and presence and exact UTF-8 bytes of the optional caption, never the source URL. One deferred provider attempt uploads those verified bytes once and performs at most one image send. Upload failure before the image send is definitive; ambiguous image-send acceptance becomes `unknown` and is never retried automatically. Source and provider upload URLs are never retained, returned, or logged.

`send_image` returns the same closed Send Operation receipt as `send_text_message`. An exact replay requires the same grant, key, connection, destination, bytes, MIME, and optional caption and never uploads or sends again. Identity-only provider evidence cannot create caption content without the image; an authenticated media-bearing outbound upsert is required to establish the Stored Message.
